            ...
            <div class="row g-2 mb-2">
              <div class="col-md-4">
                <select v-model="selectedProvince" @change="onProvinceChange" class="form-select">
                  ...
                  <option v-for="province in provinces" :key="province.ProvinceID" :value="province">
              ...
              <div class="col-md-4">
                <select v-model="selectedDistrict" @change="onDistrictChange" :disabled="!selectedProvince" class="form-select">
                  ...
                  <option v-for="district in districts" :key="district.DistrictID" :value="district">
...
export default {
  ...
  const [wardCode, districtId, provinceId] = addressIdGHN.split("/");

  // Tìm province
  this.selectedProvince = this.provinces.find(p => p.ProvinceID == provinceId) || null;
  if (this.selectedProvince) {
    this.districts = this.selectedProvince.Districts;
  }

  // Tìm district
  this.selectedDistrict = this.districts.find(d => d.DistrictID == districtId) || null;
  if (this.selectedDistrict) {
    this.wards = this.selectedDistrict.Wards;
  }

  // Tìm ward
  this.selectedWard = this.wards.find(w => w.WardCode == wardCode) || null;

  // Sau khi set thì build lại address + addressIdGHN cho chắc
  this.updateAddress();
},
    ...
    async saveFacility() {
      try {
        ...
        const payload = {
          ...
          addressIdGHN: this.form.addressIdGHN,
          type: this.form.type,
